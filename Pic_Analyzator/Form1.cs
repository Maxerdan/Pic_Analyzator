...
namespace Pic_Analyzator
{
    ...
    {
        private Bitmap _bitmap; // original picture
        private int _max; // max brightness of all pixels
        private int _min; // min brightness level
        private List<Pixel> _pixel; // pixels array from original picture
        private List<Pixel> _starPixel;
        private bool _stopPlay;
        private Color[,] arr; // array contains analyzed pixel (star pixel)
        ...
        private async void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            FillPixelsArray();
            await Task.Run(() => PixelAnalysing());
            await Task.Run(() => FillArrayAndFindMin());
        }
        ...
        private void PlayMusic()
        {
            ...
            MidiPlayer.OpenMidi();
            var starFlag = true;
            var oct = (_max - _min) / 6; // 6 - max num of octaves
            ...
            for (var w = 0; w < W; w++)
            {
                ...
                for (var i = 0; i < H; i++) redColumn.SetPixel(w, i, Color.IndianRed);
                ...
                for (var h = 0; h < H; h++)
                    if (!_stopPlay)
                    {
                        if (arr[w, h].Name != "0" && starFlag)
                        {
                            ...
                            for (var i = 2; i < 6; i++)
                                if (brightLevel > _min + oct * i && brightLevel <= _min + oct * (i + 1)
                                ) // find octave num
                                {
                                    octave = i + 1;
                                    break;
